      ...
      <div class="form-container sign-in-container">
        ...
        >
          ...
          <el-form-item class="form-item" prop="verificationCode">
            <el-input
              :prefix-icon="icon.validator"
              v-model="signInInfo.verificationCode"
              placeholder="请输入验证码"
              maxLength="6"
          ...
          <div class="button-group">
            ...
            <el-button
              class="login-button signin-button"
              type="primary"
              @click="signInSubmitForm"
              >登 录</el-button
            >
          </div>
        </el-form>
...
export default defineComponent({
    ...
    signInRelus() {
      return {
        ...
        verificationCode: [
          ...
          { validator: this.validateVerification }, // 在原来那里点击取消提交验证
        ],
      };
    },
  },
  methods: {
    ...
    signInMoveClick() {
      ...
      this.$router.replace("/api/login/signin");
      this.signInClassChange();
    },
    signUpClassChange() {
      let classList = (this.$refs.LoginViewContainer as HTMLElement)?.classList;
    ...
    signUpMoveClick() {
      ...
      this.$router.replace("/api/login/signup");
      this.signUpClassChange();
    },
    resetSignInForm() {
    ...
    getVerificationCode(codeList: string) {
      console.log(codeList);
      this.signInInfo.inputVerificationCode = codeList;
    },
    validateVerification(rule: any, value: any, callback: any) {
      if (!value) {
        return callback(new Error("填写此字段"));
      } else {
        if (value.toLowerCase() === this.signInInfo.inputVerificationCode) {
          callback();
        } else {
          return callback(new Error("输入的验证码有误，请重新输入"));
        }
      }
    },
    async signInSubmitForm() {
      let formEl = this.$refs.signInForm as FormInstance;
      if (!formEl) {
        return;
      }
      console.log(formEl);
      await formEl.validate((valid) => {
        if (valid) {
          let data = this.signInInfo;
          data.password = Md5.hashStr(this.signInInfo.password).toString(); // 密码加密
        } else {
          ElMessage({
            message: "form 提交失败",
            type: "error",
